...
import (
	...
	"github.com/scufteam/purplehack/utils"
)
...
func GenerateOrLoadRsaKeyPair() error {
	if utils.IsFileExists(privateKeyFilename) && utils.IsFileExists(publicKeyFilename) {
		privateKey, err := readPEMKey[*rsa.PrivateKey](privateKeyFilename)
		if err != nil {
			return err
		}

		publicKey, err := readPEMKey[*rsa.PublicKey](publicKeyFilename)
		if err != nil {
			return err
		}
		...
		if err == nil {
			...
			savePEMKey(privateKeyFilename, keyPair.privateKey)
			savePEMKey(publicKeyFilename, keyPair.publicKey)
		}
		return err
	}
}
